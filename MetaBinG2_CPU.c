...
void parallel(float *scorelist,float *sigma,int *index,int buff,int mrow,float alpha){
        float m;
        float tmp;
		for(int i=0;i<buff;i++){
			...
			for(int j=0;j<mrow;j++){
				//printf("%.2f\t",scorelist[i*mrow+j]);
				tmp=scorelist[i*mrow+j]*(1+sigmaG[j]*alpha);
				if(tmp<m){
					m=tmp;
					index[i]=j;
				}
...
int matMultiplication(int mrow,int buff,float* scorelist,int seqvlen,float* seqm,char ** datas,char** titlel,float* sigmaG,int* statsNum,int flag,float alpha,int* index,int buffi,FILE *fp){
	const enum CBLAS_ORDER Oder=CblasRowMajor;
	const enum CBLAS_TRANSPOSE TransA=CblasNoTrans;
	const enum CBLAS_TRANSPOSE TransB=CblasTrans;

	cblas_sgemm(Oder,TransA,TransB,buff,mrow,seqvlen,1.0f,seqm,seqvlen,datad,seqvlen,0.0,scorelist,mrow);

    int readNum;
	parallel(scorelist,sigmaG,index,buff,mrow,alpha);
            ...
            if(line[0]=='>'){
					if(title[0]=='>'){
						...
						if(buffi==buff){
							...
							matMultiplication(mrow,buff,scorelist,seqvlen,seqm,datas_fun,titlel,sigmaG_fun,statsNumthread,flag,alpha,index,buffi,fp1);
							for(int i=0;i<seqvlen*buff;i++){
								seqm[i]=0;
							}
						}
